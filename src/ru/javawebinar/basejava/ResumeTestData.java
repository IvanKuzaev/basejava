...
public class ResumeTestData {
    ...
    public static Resume fillDummyResume(Resume resume, int dummy) {
        ...
        if (Math.random() > THRESHOLD) { resume.setContact(Contacts.MOBILE_PHONE, "+7(123) 456-78" + dummy); }
        if (Math.random() > THRESHOLD) { resume.setContact(Contacts.SKYPE, "dummy.dummy" + dummy); }
        if (Math.random() > THRESHOLD) { resume.setContact(Contacts.EMAIL, "dummy" + dummy + "@dummy.ru"); }
        if (Math.random() > THRESHOLD) { resume.setContact(Contacts.HOMEPAGE, "http://dummy" + dummy + ".ru"); }
        if (Math.random() > THRESHOLD) { resume.setContact(Contacts.LINKEDIN, "https://www.linkedin.com/in/dummy" + dummy); }
        if (Math.random() > THRESHOLD) { resume.setContact(Contacts.GITHUB, "https://github.com/dummy" + dummy); }
        if (Math.random() > THRESHOLD) { resume.setContact(Contacts.STACKOVERFLOW, "https://stackoverflow.com/users/dummy" + dummy); }

        if (Math.random() > THRESHOLD) { resume.setSection(Sections.OBJECTIVE, new StringSection("Objective1_" + dummy + ", Objective2_" + dummy + ", Objective3_" + dummy)); }
        if (Math.random() > THRESHOLD) { resume.setSection(Sections.PERSONAL, new StringSection("Personal1_" + dummy + ", Personal2_" + dummy + ", Personal3_" + dummy)); }
        if (Math.random() > THRESHOLD) { resume.setSection(Sections.ACHIEVEMENTS, new StringListSection("Achievement1_" + dummy, "Achievement2_" + dummy, "Achievement3_" + dummy)); }
        if (Math.random() > THRESHOLD) { resume.setSection(Sections.QUALIFICATIONS, new StringListSection("Qualification1_" + dummy, "Qualification2_" + dummy, "Qualification3_" + dummy)); }
